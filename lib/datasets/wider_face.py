...
class wider_face(imdb):
  def __init__(self, image_set, devkit_path=None):
    ...
    self._image_ext = '.jpg'
    # 存放图片的路径索引
    self._image_index = self._load_image_set_index('wider_face.txt')
  ...
  def _load_image_set_index(self, imagelist):
    ...
    image_set_dir = 'wider_face_split'
    image_set_file = self._image_set + '/wider_face_train_bbx_gt.txt'
    ...
    with open(image_set_file) as f:
        ...
        while idx < len(lines):
            ...
            print ('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', image_name)
            # image_name = os.path.join(self._image_set, image_name)
            # print image_name
            ...
            if len(keep_idx[0]) <= 0:
                idx += 1
                # print('widths = %d,\theights = %d,\tskipping...' % (widths, heights))
                continue
            ...
            image_index.append(str(image_name))

            idx += 1

        assert(idx == len(lines))
    print('image_index = %d' % len(image_index))
    return image_index
  ...
  def _load_annotation01(self):
    ...
    gt_roidb = []
    annotationfile = os.path.join(self._data_path, 'wider_face.txt')
    f = open(annotationfile)
    split_line = f.readline().strip().split(' ')
    num = 1
    while(split_line):
        # num_objs = int(split_line[1])
        bbox = map(float, split_line[1:])
        bbboxes = np.array(bbox, dtype=np.float32).reshape(-1, 4)
        num_objs = len(bbboxes)
        print ('>>>>>>>>>>>>>>>>', num_objs)
        ...
        for i in range(1):
            ...
            x2 = float (split_line[3 + i * 4]) + x1
            y2 = float (split_line[4 + i * 4]) + y1
            ...
            gt_classes[i] = cls
            overlaps[i,cls] = 1.0
            if (boxes[i, 0]== 0) or (boxes[i, 2] < boxes[i, 0]):
                boxes[i, 0] = 10
                boxes[i, 1] = 20
                boxes[i, 2] = 30
                boxes[i, 3] = 40
                print ('bbbbbbbbbbbbbbbbbb',boxes[i])
                print (split_line[0])
